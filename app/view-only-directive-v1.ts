...
export class ViewOnlyDirective {
  ...
  updateVirtualElements(){
    ...
    if(afterVisibility.boundries.top > -100){
      ...
      if(this.virtualElements.after.length > 0){
        ...
        this.itemCount += 1
        this.inView.push(this.elements[this.itemCount-1])
        this._incrementCountTimeout = setTimeout(()=>this.main(),1)
      }
    }
  ...
  transmit(){
    let result = this.inView.slice(this.virtualElements.before.length,this.inView.length - this.virtualElements.before.length - this.virtualElements.after.length)
    console.clear()
    console.log("In view length:",this.inView.length)
